...
namespace ModelTest
{
    ...
    public class CollectionTests : PublicationBaseChildTests<Collection>
    {
        ...
        public void ResponsibleEditorsAssertionTest(string responsibleEditors)
        {
            var collection = new Collection();
        ...
        public void GetGOSTInformationFullDataTest()
        {
            ...
            string result = collection.GetGOST();
            Assert.Multiple(() =>
            {
                Assert.That(result,
                    Does.Contain("Актуальные проблемы информатики"),
                    "Заголовок не найден");
                Assert.That(result,
                    Does.Contain("сборник статей"),
                    "Подзаголовок не найден");
                Assert.That(result,
                    Does.Contain("Иванов И.И., Петров П.П."),
                    "Редколлегия не найдена");
                Assert.That(result,
                    Does.Contain("Сидоров С.С."),
        ...
        public void GetGOSTInformationMinimalDataTest()
        {
            ...
            {
                Assert.That(result,
                    Does.Contain("Минимальный сборник"),
                    "Заголовок не найден");
                Assert.That(result,
                    Does.Contain("Редактор А.А."),
                    "Редколлегия не найдена");
                Assert.That(result,
                    Does.Contain("Отв. ред. Б.Б."),
                    "Ответственный редактор не найден");
                Assert.That(result,
                    Does.Contain("50 с."),
                    "Страницы не найдены");

...
namespace ModelTest
{
    ...
    /// </summary>
    [TestFixture]
    public class DissertationTests : PublicationBaseChildTests<Dissertation>
    {
        /// <summary>
        ...
        {
            var dissertation = new Dissertation();
            Assert.Throws<ArgumentException>(() => dissertation.AuthorFull = invalidAuthor);
        }

        /// <summary>
        /// Проверка корректных данных для свойства Speciality
        /// </summary>
        [TestCase("05.13.01 - Системный анализ", TestName = "Полная специальность")]
        [TestCase("01.01.01", TestName = "Код специальности")]
        [TestCase("Математическое моделирование", TestName = "Название специальности")]
        ...
        {
            var dissertation = new Dissertation();
            Assert.Throws<ArgumentException>(() => dissertation.Speciality = invalidSpeciality);
        }
        ...
        public void GetGOSTFullDataTest()
        {
            ...
            string result = dissertation.GetGOST();
            Assert.Multiple(() =>
            {
                Assert.That(result, Does.Contain("Иванов, И."), "Автор в заголовке не найден");
                Assert.That(result, Does.Contain("Методы анализа данных"), "Заголовок не найден");
                Assert.That(result, Does.Contain("специальность 05.13.01"), "Специальность не найдена");
                Assert.That(result, Does.Contain("Кандидат наук"), "Степень не найдена");
                Assert.That(result, Does.Contain("Иванов Иван Иванович"), "Автор в подзаголовке не найден");
                Assert.That(result, Does.Contain("МГУ"), "Издательство не найдено");
                Assert.That(result, Does.Contain("Москва"), "Место не найдено");
                Assert.That(result, Does.Contain("2023"), "Год не найден");
                Assert.That(result, Does.Contain("150 с."), "Страницы не найдены");
            });
        }
        ...
        public void GetGOSTMinimalDataTest()
        {
            ...
            Assert.Multiple(() =>
            {
                Assert.That(result, Does.StartWith("Петров"), "Автор не в начале");
                Assert.That(result, Does.Contain("Простая диссертация"), "Заголовок не найден");
                Assert.That(result, Does.Contain("Доктор наук"), "Степень не найдена");
                Assert.That(result, Does.Contain("100 с."), "Страницы не найдены");
            });
        }
    }
}